...
imageFrame = tk.Label(imageFrame)
...
Socket_val  =  tk.BooleanVar()
Serial_val =  tk.BooleanVar()
...
X_HeadPos_val = tk.IntVar(value=X_HeadPos)
Y_HeadPos_val = tk.IntVar(value=Y_HeadPos)
EB_color = (255, 100, 50)
...
def Head_Tracking(Px, Py):
    ...
    if (samples == 10):
        ...
        # Output buffer
        serout = (b'<%+0.3d|%+0.3d>' % (result_x, result_y))
...
def CaptureVideo():
    ...
    if (camera.isOpened()):
        ...
        x1 = int((cam_width - CroppingWidth_val) / 2)
        y1 = int((cam_height - CroppingHeight_val) / 2)
        x2 = int(x1 + CroppingWidth_val)
        y2 = int(y1 + CroppingHeight_val)
        ...
        cv2.putText( crop, "{}x{}".format( CroppingWidth_val, CroppingHeight_val ), (5, 20), cv2.FONT_HERSHEY_SIMPLEX,
        ...
        cv2.line( crop, (0, CroppingHeight_val / 2), (CroppingWidth_val, CroppingHeight_val / 2), Overlay1_color, 1,
                  cv2.LINE_AA )
        cv2.line( crop, (CroppingWidth_val / 2, 0), (CroppingWidth_val / 2, CroppingHeight_val), Overlay1_color, 1,
                  cv2.LINE_AA )
        cv2.putText( crop, "Head Pos : x{} y{}".format( int( headpos_x ), int( headpos_y ) ),
                     (5, (CroppingHeight_val - 20)), cv2.FONT_HERSHEY_SIMPLEX, 0.7, Overlay2_color, 1, cv2.LINE_AA )
        cv2.rectangle( crop, (CroppingWidth_val / 2 - 10, CroppingHeight_val / 2 - 10),
                       (CroppingWidth_val / 2 + 10, CroppingHeight_val / 2 + 10), Overlay1_color, 1 )
...
###Control Frame
Controls = tk.LabelFrame( imageFrame, text='Controls', font="Consolas 11 bold" )
Controls.grid( row=0, column=0, rowspan=10, ipadx=5, ipady=5, sticky='nesw' )

###BlankSpace
blankspace = tk.Label( Controls )
blankspace.grid( row=0, column=0, sticky='we' )

###HitBoxFrame
HitBoxFrame = tk.LabelFrame( Controls, text='HitBox', font="Consolas 10 bold" )
HitBoxFrame.grid( row=1, column=0, sticky='nesw', ipadx=5, ipady=5, columnspan=1 )
tk.Label( HitBoxFrame, text="Min Size", font="Consolas 10" ).grid( row=0, column=1, sticky='ws' )
HeadMinSizeScaler = tk.Scale( HitBoxFrame, from_=20, to=200, resolution=10, orient=tk.HORIZONTAL )
HeadMinSizeScaler.grid( row=0, column=0, sticky='w' )
HeadMinSizeScaler.set( HeadMinSize_def_val )
tk.Label( HitBoxFrame, text="Max Size", font="Consolas 10" ).grid( row=1, column=1, sticky='ws' )
HeadMaxSizeScaler = tk.Scale( HitBoxFrame, from_=200, to=600, resolution=10, orient=tk.HORIZONTAL )
HeadMaxSizeScaler.grid( row=1, column=0, sticky='w' )
HeadMaxSizeScaler.set( HeadMaxSize_def_val )

###Cropping Frame
CroppingFrame = tk.LabelFrame( Controls, text='Cropping', font="Consolas 10 bold" )
CroppingFrame.grid( row=2, column=0, sticky='nesw', ipadx=5, ipady=5, columnspan=1 )
tk.Label( CroppingFrame, text="Width ", font="Consolas 10" ).grid( row=0, column=1, sticky='ws' )
CroppingWidthScaler = tk.Scale( CroppingFrame, from_=int( cam_width / 2 ), to=cam_width, resolution=10, orient=tk.HORIZONTAL )
CroppingWidthScaler.grid( row=0, column=0, sticky='w' )
CroppingWidthScaler.set( cropping_width_def_val )
tk.Label( CroppingFrame, text="Height", font="Consolas 10" ).grid( row=1, column=1, sticky='ws' )
CroppingHeightScaler = tk.Scale( CroppingFrame, from_=int( cam_height / 2 ), to=cam_height, resolution=10,
                              orient=tk.HORIZONTAL )
CroppingHeightScaler.grid( row=1, column=0, sticky='w' )
CroppingHeightScaler.set( cropping_height_def_val )

###SpecialFrame
SpecialFrame = tk.LabelFrame( Controls, text='Special', font="Consolas 10 bold" )
SpecialFrame.grid( row=3, column=0, sticky='nesw', ipadx=5, ipady=5 )
tk.Label( SpecialFrame, text="MinNeighbors", font="Consolas 10" ).grid( row=0, column=1, sticky='ws' )
minNeighborsScaler = tk.Scale( SpecialFrame, from_=1, to=10, orient=tk.HORIZONTAL )
minNeighborsScaler.grid( row=0, column=0, sticky='we' )
minNeighborsScaler.set( minNeighbors_def_val )
tk.Label( SpecialFrame, text="ScaleFactor", font="Consolas 10" ).grid( row=1, column=1, sticky='ws' )
scaleFactorScaler = tk.Scale( SpecialFrame, from_=1.02, to=1.6, resolution=0.02, orient=tk.HORIZONTAL )
scaleFactorScaler.grid( row=1, column=0, sticky='we' )
scaleFactorScaler.set( scaleFactor_def_val )

### HeadResetbutton
HeadResetbutton = tk.Button( Controls, width=16, height=1, text='Reset params', font="Consolas 10", fg="red",
...
###SerialComFrame
SerialComFrame = tk.LabelFrame( Controls, text='SerialCom', font="Consolas 10 bold" )
SerialComFrame.grid( row=6, column=0, sticky='nesw', ipadx=5, ipady=5, columnspan=1 )
Serialbutton = tk.Checkbutton( SerialComFrame, width=20, height=1, text='Send position', font="Consolas 10",
                            variable=Serial_val )
...
###SocketFrame
SocketFrame = tk.LabelFrame( Controls, text='SocketCom', font="Consolas 10 bold" )
SocketFrame.grid( row=7, column=0, sticky='nesw', ipadx=5, ipady=5, columnspan=1 )
Socketbutton = tk.Checkbutton( SocketFrame, width=20, height=1, text='Send position', font="Consolas 10",
                            variable=Socket_val )
...
### CameraFrame
CameraFrame = tk.LabelFrame( imageFrame, width=cam_width, height=cam_height )
...
###HeadBox Frame
HeadBoxFrame = tk.LabelFrame( imageFrame, text='HeadBox Position', font="Consolas 11 bold" )
HeadBoxFrame.grid( row=0, column=1, columnspan=2, sticky='nesw' )
tk.Label( HeadBoxFrame, text=" X =", font="Consolas 12 bold" ).grid( row=0, column=0, sticky=tk.W )
tk.Label( HeadBoxFrame, text=" Y =", font="Consolas 12 bold" ).grid( row=1, column=0, sticky=tk.W )
tk.Label( HeadBoxFrame, textvariable=X_HeadPos_val, font="Consolas 12 bold" ).grid( row=0, column=1, sticky=tk.W )
tk.Label( HeadBoxFrame, textvariable=Y_HeadPos_val, font="Consolas 12 bold" ).grid( row=1, column=1, sticky=tk.W )
